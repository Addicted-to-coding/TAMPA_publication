...
def generateFigure(PF, sample, rank, input_file, output_base_name, output_path,file_type, plot_l1, scaling, output_dpi, use_profile, fw, fh, limit=None, limit_value=None):
    ...
    if len(PF.ground_truth_dict) > 0:
        ...
        T2 = TextFace("Truth", fsize=legend_fs)
        T2.hz_align = True
    ...
    tree_output_file = f"{output_path}/{output_base_name}_tree_{rank}_{sample}.{file_type}"
...
def main():
    ...
    for sample in sample_keys:
        generateFigure(PF, sample, rank, input_file, output_base_name, output_path, file_type, plot_l1, scaling, output_dpi, use_profile, fw, fh, limit, limit_value)
